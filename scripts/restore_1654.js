...
// Fix 1: Status text color for "ご予約完売"
c = c.replace(
...
// Fix 2: Quoted Review link pointing to tab=posted_reviews
if (c.includes('href={`/cast/${quotedReview.reviewer_id}`}') && c.includes('className="shrink-0"')) {
   c = c.replace(
...
// Fix 3: Fullscreen Quoted Review link
if (c.includes('onClick={() => setFullscreenMedia(null)}')) {
    c = c.replace(
...
console.log("Successfully restored 16:54 design and applied all previous minor patches.");
