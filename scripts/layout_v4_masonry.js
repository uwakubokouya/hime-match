...
const headerLines = lines.slice(0, 169); // up to `return (` (inclusive)
      ...
      {!isDeletedLocally && (
        <article className="break-inside-avoid mb-3 border border-[#E5E5E5] rounded-xl bg-white shadow-sm overflow-hidden flex flex-col relative">
            ...
            {images.length > 0 && (
                ...
                >
                    {images[0].match(/\\.(mp4|mov|webm)$/i) ? (
                        <>
                            <video src={images[0]} className={\`w-full h-auto object-cover transition-all duration-700 pointer-events-none \${shouldBlur ? 'blur-xl scale-110' : ''}\`} autoPlay loop muted playsInline />
                            {!shouldBlur && (
                        ...
                        <img src={images[0]} className={\`w-full h-auto object-cover transition-all duration-700 \${shouldBlur ? 'blur-xl scale-110' : ''}\`} loading="lazy" />
                    )}
                    
                    {!shouldBlur && <MediaWatermark />}
                    {!shouldBlur && images.length > 1 && (
                        <div className="absolute bottom-2 right-2 z-20 pointer-events-none bg-black/60 backdrop-blur-sm p-1.5 rounded-md shadow-md text-white flex items-center gap-1">
                            <Layers size={14} />
            ...
            <div className="p-3 flex flex-col gap-2">
                {/* Post Text */}
                <p className={\`text-[11px] text-[#333333] leading-relaxed whitespace-pre-wrap break-words font-light \${localIsLocked ? 'blur-[4px] select-none pointer-events-none' : ''}\`}>
                    {content}
                </p>

                {/* Quoted Review / Tagged Cast (Compact Pill Style) */}
                {quotedReview && (
                    <Link href={\`/cast/\${quotedReview.reviewer_id}?tab=posted_reviews\`} className={\`flex items-center justify-between gap-2 border border-[#E5E5E5] bg-[#F9F9F9] rounded-lg p-2 hover:bg-[#F0F0F0] transition-colors \${localIsLocked ? 'blur-[4px] select-none pointer-events-none' : ''}\`}>
                ...
                {taggedCast && (
                    <Link href={\`/cast/\${taggedCast.id}\`} className={\`flex items-center gap-2 border border-[#E5E5E5] bg-[#F9F9F9] rounded-lg p-2 hover:bg-[#F0F0F0] transition-colors \${localIsLocked ? 'blur-[4px] select-none pointer-events-none' : ''}\`}>
                ...
                {/* Business Banner Pill */}
                {isWorkingToday && (
                    <div className="border border-[#E5E5E5] bg-[#F9F9F9] p-2 rounded-lg relative mt-1">
                        <div className="flex flex-col gap-1">
                            <div className="flex items-center gap-1">
                                <span className={\`w-1.5 h-1.5 rounded-none \${statusText === '受付終了' ? 'bg-[#777777]' : statusText === 'ご予約完売' ? 'bg-[#333333]' : 'bg-[#E02424] animate-pulse'}\`}></span>
                                <span className={\`font-bold text-[9px] tracking-widest \${statusText === '受付終了' ? 'text-[#777777]' : statusText === 'ご予約完売' ? 'text-[#333333]' : 'text-[#E02424]'}\`}>
                                    {statusText || "本日出勤中"}
                                </span>
                            </div>
                            {nextAvailableTime && (
                                <div className="text-[9px] text-[#777777] font-light tracking-widest flex items-center gap-1">
                                    <Clock size={10} className={nextAvailableTime === '待機中' ? 'text-[#E02424]' : 'text-[#777777]'} />
                                    {nextAvailableTime.startsWith('次回出勤') ? nextAvailableTime : \`次回の空き時間: \${nextAvailableTime}\${nextAvailableTime !== '待機中' ? '〜' : ''}\`}
                                </div>
                            )}
                        </div>
                        {statusText !== '受付終了' && statusText !== 'ご予約完売' && (
                            <Link href={\`/reserve/\${castId}\`} className="mt-2 bg-[#FF5C8A] text-white flex items-center justify-center py-1.5 rounded-md text-[9px] font-bold tracking-widest hover:bg-[#E04877] transition-colors w-full shadow-sm">
